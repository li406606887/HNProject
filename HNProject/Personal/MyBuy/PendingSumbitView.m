...
@interface PendingSumbitView()<UITableViewDelegate,UITableViewDataSource>
@property(nonatomic,strong) MyBuyViewModel *viewModel;
@property(nonatomic,strong) UITableView *table;
@end

@implementation PendingSumbitView
...
-(void)bindViewModel{
    @weakify(self)
    [self.viewModel.sumbitDataCommand execute:nil];
    [self.viewModel.sumbitRefreshUISubject subscribeNext:^(id  _Nullable x) {
     @strongify(self)
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    ...
    return self.viewModel.sumbitArray.count;
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.stateString = [NSString stringWithFormat:@"审核已通过,待提交"];
    [cell.operationButton setTitle:@"提交" forState:UIControlStateNormal];
    cell.model = self.viewModel.sumbitArray[indexPath.row];
    cell.viewModel = self.viewModel;
    return cell;
}
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    MyBuyModel *model = self.viewModel.sumbitArray[indexPath.row];
    [self.viewModel.sumbitCellClickSubJect sendNext:model];
}
/*
...
-(UITableView *)table {
    if (!_table) {
        ...
        _table.mj_header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
            @strongify(self)
            [self.viewModel.sumbitDataCommand execute:@"0"];
        }];
