...
@interface CompletedView()<UITableViewDelegate,UITableViewDataSource>
...
-(void)bindViewModel{
    @weakify(self)
    [self.viewModel.completedCommand execute:nil];
    [self.viewModel.completedRefreshUISubject subscribeNext:^(id  _Nullable x) {
        @strongify(self)
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    NSLog(@"%ld",self.viewModel.completedArray.count);
    return self.viewModel.completedArray.count;
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.stateString = [NSString stringWithFormat:@"已审核通过"];
    cell.operationButton.hidden = YES;
    cell.model = self.viewModel.completedArray[indexPath.row];
    return cell;
}
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    MyBuyModel *model = self.viewModel.completedArray[indexPath.row];
    [self.viewModel.completedCellClickSubJect sendNext:model];
}
/*
...
-(UITableView *)table {
    if (!_table) {
        ...
        _table.mj_header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
            @strongify(self)
            [self.viewModel.completedCommand execute:@"0"];
        }];
